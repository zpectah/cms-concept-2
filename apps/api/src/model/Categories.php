...
class Categories extends Model {
  ...
  public function get_list(): array {
    ...
    $sql = "SELECT id, name, type, parent_id, active, deleted, created, updated FROM `categories`";
  ...
  public function create($data, $locales): array {
    ...
    $params = self::get_columns_and_values_for_query([
      'type', 'name', 'parent_id', 'active', 'deleted'
    ]);
    ...
    if (isset($data['locale']) && is_array($data['locale'])) {
      $localeParams = self::get_columns_and_values_for_query([ 'title', 'description', 'id' ]);
  ...
  public function patch($data, $locales): array {
    ...
    if (isset($data['locale']) && is_array($data['locale'])) {
      foreach ($locales as $locale) {
        if (isset($data['locale'][$locale])) {
          $localeData = self::parse_locale_row($data['locale'][$locale]);
          $localeSetParts = self::query_parts($localeData, [ 'title', 'description', 'id' ]);
          $tableName = 'categories_' . $locale;
  ...
  public function delete($data): array {
    ...
    $sql = "UPDATE `categories` SET `deleted` = NOT `deleted` WHERE `id` IN ({$placeholders})";
    $stmt = $conn -> prepare($sql);
    $stmt -> execute($data);
