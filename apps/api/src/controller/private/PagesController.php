...
class PagesController extends Controller {
  ...
  private function create($url, $data): array {
    ...
    return self::$pages -> create($data, $locales);
  }

  private function patch($url, $data): array {
    $locales = self::getActiveLocales();

    return self::$pages -> patch($data, $locales);
  }

  private function toggle($url, $data): array {
    return self::$pages -> toggle($data);
  }

  private function delete($url, $data): array {
    return self::$pages -> delete($data);
  }

  private function deletePermanent($url, $data): array {
    $locales = self::getActiveLocales();

    return self::$pages -> delete_permanent($data, $locales);
  }
