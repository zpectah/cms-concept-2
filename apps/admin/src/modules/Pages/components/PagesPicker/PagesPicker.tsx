...
import { PagesPickerProps } from './types';

const PagesPicker = forwardRef<HTMLInputElement, PagesPickerProps>(
  (props, ref) => {
    const { value, multiple, ignored = [], ...rest } = props;
    ...
    const options = useMemo(() => {
      const rows = [];
      const filtered = (pages ?? []).filter((item) => !item.deleted);
